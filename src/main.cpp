...
#if defined(NDEBUG)
#error "PTCL cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uPTCL) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("ptclcoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        const bool hasPTCLInputs = !bibInputs.empty();
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // PTCL: We use certain sporks during IBD, so check to see if they are
