...
#include "qt/ptclcoin/dashboardwidget.h"
#include "qt/ptclcoin/forms/ui_dashboardwidget.h"
#include "qt/ptclcoin/sendconfirmdialog.h"
#include "qt/ptclcoin/txrow.h"
...
DashboardWidget::DashboardWidget(PTCLGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of PTCL earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 PTCL");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &PTCLGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &PTCLGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &PTCLGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("PTCL")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(48, 48, 48);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair PTCL, zPTCL
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(130, 196, 108));
	set1->setColor(QColor(15, 15, 15));
