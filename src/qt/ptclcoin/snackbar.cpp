...
#include "qt/ptclcoin/forms/ui_snackbar.h"
#include "qt/ptclcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(PTCLGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &PTCLGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
