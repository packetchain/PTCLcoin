...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PTCL);
    ...
    if (settings.contains("nAnonymizePtclAmount") || reset)
        SoftSetArg("-anonymizeptclcoinamount", settings.value("nAnonymizePtclAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ptclcoin.cpp during shotdown to also remove the window positions
