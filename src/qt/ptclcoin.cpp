...
#include "config/ptclcoin-config.h"
#endif
...
#include "qt/ptclcoin/splash.h"
#include "qt/ptclcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ptclcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ptclcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ptclcoin.qrc)
...
/** Class encapsulating PTCL Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (PTCLGUI)
...
{
    window = new PTCLGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PTCLGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PTCLGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(PTCLGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PTCLGUI::DEFAULT_WALLET);
        ...
        // PTCL: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PTCLGUI::handlePaymentRequest);
        connect(window, &PTCLGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. PTCL can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ptclcoin_locale);
    Q_INIT_RESOURCE(ptclcoin);
    ...
    /// 6. Determine availability of data directory and parse ptclcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("PTCL"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("PTCL"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ptclcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("PTCL didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
