...
from test_framework.test_framework import PtclTestFramework
...
class ReorgStakeTest(PtclTestFramework):
    ...
    def check_money_supply(self, expected_bib, expected_zbib):
        ...
        # verify that nodes have the expected PTCL and zPTCL supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bib))
            for denom in node['zPTCLsupply']:
                assert_equal(node['zPTCLsupply'][denom], DecimalAmt(expected_zbib[denom]))
    ...
    def run_test(self):
        ...
        # Check PTCL and zPTCL supply at the beginning
        # ------------------------------------------
        # zPTCL supply: 2 coins for each denomination
        ...
        # PTCL supply: block rewards minus burned fees for minting
        ...
        # Verify that PTCL and zPTCL supplies were properly updated after the spends and reorgs
        self.log.info("Check PTCL and zPTCL supply...")
