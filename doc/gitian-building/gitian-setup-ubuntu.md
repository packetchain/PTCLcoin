...
The build script we'll be using is contained in the PTCL github repository ([contrib/gitian-build.py](https://github.com/ptclcoin910/ptclcoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the PTCL repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/PTCL-Project/PTCL/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, ptclcoin-detached-sigs, and ptclcoin GitHub repos.
