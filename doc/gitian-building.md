...
*Setup instructions for a gitian build of PTCL Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the PTCL
...
to the PTCL GitHub release page.
...
- [Building PTCL Core](#building-ptclcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ptclcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ptclcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ptclcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ptclcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ptclcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ptclcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ptclcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ptclcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ptclcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ptclcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ptclcoin-osx-*-build.assert
```
